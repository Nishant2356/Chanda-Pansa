...
(function initSocket() {
    function setup() {
        ...
        socket.on('gotiMoved', (moveData) => {
            // apply the incoming full-board HTML and state snapshot
            document.querySelector(".app").innerHTML = moveData.board;
            document.querySelector(".turnBox").innerHTML = moveData.turnBox;
            ...
            assignedColor = moveData.assignedColor;
            // IMPORTANT: rebind DOM references & event listeners after you replaced innerHTML
    ...
    if (typeof io !== "undefined") {
        ...
        window.addEventListener("load", () => {
            if (typeof io !== "undefined") {
                ...
                console.warn('Socket.IO client not found. Make sure <script src="/socket.io/socket.io.js"></script> is loaded BEFORE script.js');
            }
        });
    }
})();
...
btn.addEventListener("click", (e) => {
    if (assignedColor[socket.id] !== turn[currentTurn]) {
        e.preventDefault(); // stops default action (form submit, etc.)
        return; // exits without running your click logic
    }
